  ...
  <div class="p-2">
    ...
    <el-card shadow="never" style="height: 78vh; overflow-y: auto">
      <el-form ref="leaveFormRef" v-loading="loading" :disabled="routeParams.type === 'view'" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="加班类型" prop="leaveType">
          <el-select v-model="form.leaveType" placeholder="请选择加班类型" style="width: 100%">
        ...
        <el-form-item label="加班天数" prop="leaveDays">
          <el-input v-model="form.leaveDays" disabled type="number" placeholder="请输入加班天数" />
        </el-form-item>
        <el-form-item label="加班原因" prop="remark">
          <el-input v-model="form.remark" type="textarea" :rows="3" placeholder="请输入加班原因" />
        </el-form-item>
      </el-form>
    </el-card>
    <!-- 提交组件 -->
...
import { addLeave, getLeave, updateLeave } from '@/api/workflow/overtime';
...
const changeLeaveTime = () => {
  if (leaveTime.value && leaveTime.value.length === 2) {
    ...
    // 保留2位小数
    form.value.leaveDays = parseFloat(totalHours.toFixed(2));
    
    // 设置开始和结束时间（确保时分秒被保留）
    form.value.startDate = proxy.parseTime(startDate, '{y}-{m}-{d} {h}:{i}:{s}');
    form.value.endDate = proxy.parseTime(endDate, '{y}-{m}-{d} {h}:{i}:{s}');
  }
};
/** 获取详情 */
const getInfo = () => {
