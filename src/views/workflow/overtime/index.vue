  ...
  <div class="p-2">
    <transition :enter-active-class="proxy?.animate.searchAnimate.enter" :leave-active-class="proxy?.animate.searchAnimate.leave">
      <div v-show="showSearch" class="search">
        <el-form ref="queryFormRef" :model="queryParams" :inline="true">
          <el-form-item label="加班天数" prop="startLeaveDays">
            <el-input v-model="queryParams.startLeaveDays" placeholder="请输入加班天数" clearable @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item prop="endLeaveDays"> 至 </el-form-item>
          <el-form-item prop="endLeaveDays">
            <el-input v-model="queryParams.endLeaveDays" placeholder="请输入加班天数" clearable @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item>
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" border :data="leaveList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="加班类型" align="center">
        ...
        <el-table-column label="开始时间" align="center" prop="startDate">
            <template #default="scope">
                <span>{{ proxy.parseTime(scope.row.startDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
            </template>
        </el-table-column>
        <el-table-column label="结束时间" align="center" prop="endDate">
            <template #default="scope">
                <span>{{ proxy.parseTime(scope.row.endDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
            </template>
        </el-table-column>
        <el-table-column label="加班时间" align="center" prop="leaveDays" />
        <el-table-column label="加班原因" align="center" prop="remark" />
...
import { delLeave, listLeave } from '@/api/workflow/overtime';
import { cancelProcessApply } from '@/api/workflow/instance';
import { LeaveForm, LeaveQuery, LeaveVO } from '@/api/workflow/overtime/types';
...
const handleAdd = () => {
  ...
  proxy.$router.push({
    path: `/workflow/overtimeEdit/index`,
    query: {
      type: 'add'
    }
  });
