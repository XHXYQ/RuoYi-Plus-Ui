    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="regularApplicationList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="审批状态" align="center" prop="approvalStatus">
          <template #default="scope">
            <dict-tag :options="wf_business_status" :value="scope.row.approvalStatus" />
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="200">
          <template #default="scope">
            ...
            <el-button 
              v-if="scope.row.approvalStatus === 'draft' || scope.row.approvalStatus === 'back'" 
              size="small" 
              type="primary" 
              icon="Edit" 
            ...
            >删除</el-button>
            <el-button 
              v-if="scope.row.approvalStatus === 'waiting'" 
              size="small" 
              type="warning" 
              icon="Notification" 
...
// }
const handleAdd = () => {
  loading.value = false; // 主动取消 loading（防止因动画或 loading 卡住）
  proxy.$tab.closePage(proxy.$route);
  proxy.$router.push({
    path: `/workflow/applicationEdit/index`,
    query: {
